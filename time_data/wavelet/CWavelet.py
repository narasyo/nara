...
class CWavelet(object):
    ...
    def _get_freq_log(self, freq_range):
        ...
        k0 = int(self.n_per_octave * np.log2(freq_range[0] / 1)) # minimum key
        kn = int(self.n_per_octave * np.log2(freq_range[1] / 1)) # maximum key
        k = np.arange(k0, kn + 1, dtype=np.float64)
        freq_period = 2 ** (k / self.n_per_octave) # frequency
        return freq_period
